...
-- Replace the initial record RPC so the remote payload's entire sync object is
-- internally consistent with the server revision it returns. A later conflict
-- must always deserialize as a valid canonical record.
