...
describe('capture adapter contract', () => {
  it('creating twice with the same submission id yields one record', async () => {
    ...
    const input = { submissionId: 'sub-1', text: 'a', capturedAt: 'now', sealedAt: 'now', hasVoice: false, originalMediaIds: [], recordType: 'incident' as const };
  ...
  it('reports failed media without losing the sealed record', async () => {
    ...
    const { recordId } = await adapter.createRecord({
      submissionId: 'sub-2', text: 'kept', capturedAt: 'n', sealedAt: 'n', hasVoice: false, originalMediaIds: [], recordType: 'incident' as const,
    });
  ...
  it('retrying only the failed items can succeed later', async () => {
    ...
    const { recordId } = await failing.adapter.createRecord({
      submissionId: 'sub-3', text: 't', capturedAt: 'n', sealedAt: 'n', hasVoice: false, originalMediaIds: [], recordType: 'incident' as const,
    });
  ...
  it('review details are optional and can be skipped', async () => {
    const { adapter, records } = makeFakeAdapter();
    const { recordId } = await adapter.createRecord({
      submissionId: 'sub-4', text: 't', capturedAt: 'n', sealedAt: 'n', hasVoice: false, originalMediaIds: [], recordType: 'incident' as const,
    });
  ...
  it('a failing review save leaves the sealed record intact', async () => {
    ...
    const { recordId } = await adapter.createRecord({
      submissionId: 'sub-5', text: 'original wording', capturedAt: 'n', sealedAt: 'sealed-ts', hasVoice: false, originalMediaIds: [], recordType: 'incident' as const,
    });
