...
const cleanSnapshot = (): V1Snapshot => ({
  ...
  evidence: [{
    id: 'media-1', incident_id: 'record-1', file_name: 'photo.jpg', file_path: 'owner-1/photo.jpg',
    file_hash: LEGACY_MEDIA_SHA256, mime_type: 'image/jpeg', upload_date: at(3), file_size: LEGACY_MEDIA_BYTES.byteLength,
  } as V1Snapshot['evidence'][number]],
  history: [{
    id: 'history-1', incident_id: 'record-1', field_changed: 'title', changed_at: at(4), edit_source: 'user',
    old_value: 'Old', new_value: 'Meeting',
...
describe('Phase 5 — canonical activation guard', () => {
  ...
  beforeEach(() => {
    db = new ChronicleDB(`chronicle_phase5_${crypto.randomUUID()}`);
  });
  ...
  it('stays unactivated until every expected canonical row and evidence byte is verified', async () => {
    ...
    if (!metadataOnly.ok) expect(metadataOnly.reasons).toContain('media:media-1:verified local bytes are missing');
  ...
  it('blocks activation on unresolved migration issues even when mapped rows were written', async () => {
    const source = cleanSnapshot();
    source.incidents[0].incident_time = 'sometime after lunch';
  ...
  it('blocks activation if canonical data changed or an unexpected row appeared before cutover', async () => {
    ...
    await db.canonical_people.add({
      id: 'extra-person', owner_id: 'owner-1', display_name: 'Extra', normalised_name: 'extra',
  ...
  it('treats malformed or foreign activation receipts as inactive', async () => {
    ...
    await db.meta.put({
      key: canonicalActivationKey('owner-1'),
      value: JSON.stringify({ version: 1, owner_id: 'owner-2', state: 'canonical', activated_at: at(0), counts: {}, inspected: {} }),
    });
    expect(await getCanonicalActivation('owner-1', db)).toBeNull();
  });

  it('routes to legacy by default and only canonical after an activation receipt exists', async () => {
    const legacyGet = vi.fn(async () => null);
    const legacyList = vi.fn(async () => []);
    const canonicalGet = vi.fn(async () => null);
    const canonicalList = vi.fn(async () => []);
    const legacy: CanonicalRecordReader = { get: legacyGet, list: legacyList };
    const canonical: CanonicalRecordReader = { get: canonicalGet, list: canonicalList };
    const router = createCanonicalReadRouter(legacy, canonical, ownerId => getCanonicalActivation(ownerId, db));

    await router.list('owner-1');
    expect(legacyList).toHaveBeenCalledTimes(1);
    expect(canonicalList).not.toHaveBeenCalled();

    const build = buildCanonicalMigration(cleanSnapshot());
    await applyCanonicalMigration(build, db);
    await hydrateCanonicalMigrationMedia(build, 'owner-1', db, legacyMediaDownload, () => at(9));
    await activateCanonicalOwner(build, 'owner-1', db, () => at(10));

    await router.get('owner-1', 'record-1');
    expect(canonicalGet).toHaveBeenCalledTimes(1);
