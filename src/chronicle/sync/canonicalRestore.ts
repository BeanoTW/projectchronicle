...
type RemoteChildRow = { kind: 'clarification' | 'media' | 'history' | 'person' | 'relationship' | 'organisation'; id: string; record_id: string | null; payload: unknown; remote_revision: number; updated_at: string };
...
export async function restoreCanonicalOwner(ownerId: string, clock: () => string = () => new Date().toISOString()): Promise<CanonicalRestoreReport> {
  ...
  for (const child of children) {
    assertChildIdentity(child, ownerId); validateRemoteChildPayload(child);
    ...
    if (child.kind === 'relationship') { const relationship = RemoteRelationshipSchema.parse(child.payload); if (!remoteEntities.has(`${relationship.entity_type}:${relationship.entity_id}`)) throw new CanonicalCloudConflictError(`Remote relationship ${child.id} points to a missing ${relationship.entity_type}.`); }
  }
  let recordsAdded = 0, recordsAlreadyLocal = 0;
  for (const remote of records) { const local = await localDB.canonical_records.get(remote.id); if (!local) continue; const parsed = V2RecordSchema.parse(local); if (parsed.owner_id !== ownerId || !same(withoutRecordSync(parsed), withoutRecordSync(remote))) throw new CanonicalCloudConflictError(`Local canonical record ${remote.id} differs from its remote copy; restore will not overwrite it.`); recordsAlreadyLocal++; }
  ...
  let childrenAdded = 0, childrenAlreadyLocal = 0;
  await localDB.transaction('rw', [localDB.canonical_records, localDB.canonical_clarifications, localDB.canonical_media, localDB.canonical_blobs, localDB.canonical_history, localDB.canonical_people, localDB.canonical_organisations, localDB.canonical_relationships], async () => {
    for (const remote of records) { if (await localDB.canonical_records.get(remote.id)) continue; await localDB.canonical_records.add(remote); recordsAdded++; }
    for (const child of children) {
      const payload = child.payload as Record<string, unknown>;
      if (child.kind === 'media') { const prepared = preparedMedia.get(child.id)!; if (!prepared.hadMedia) childrenAdded++; else childrenAlreadyLocal++; await localDB.canonical_media.put(prepared.media); if (prepared.blob) await localDB.canonical_blobs.put(prepared.blob); continue; }
      if (child.kind === 'clarification') { const existing = await localDB.canonical_clarifications.get(child.id); const body = RemoteClarificationBodySchema.parse(payload); const restored: V2Clarification = { ...body, sync: restoredSync(child.remote_revision, at) }; if (existing) { const { sync: _a, ...existingBody } = existing; const { sync: _b, ...remoteBody } = restored; if (!same(existingBody, remoteBody)) throw new CanonicalCloudConflictError(`Local clarification ${child.id} differs from remote.`); childrenAlreadyLocal++; } else { await localDB.canonical_clarifications.add(restored); childrenAdded++; } continue; }
      if (child.kind === 'history') { const remote = RemoteRecordEventSchema.parse(payload); const existing = await localDB.canonical_history.get(child.id); if (existing && !same(existing, remote)) throw new CanonicalCloudConflictError(`Local history ${child.id} differs from remote.`); if (existing) childrenAlreadyLocal++; else { await localDB.canonical_history.add(remote); childrenAdded++; } continue; }
      if (child.kind === 'person') { const remote = RemotePersonSchema.parse(payload); const existing = await localDB.canonical_people.get(child.id); if (existing && !same(existing, remote)) throw new CanonicalCloudConflictError(`Local person ${child.id} differs from remote.`); if (existing) childrenAlreadyLocal++; else { await localDB.canonical_people.add(remote); childrenAdded++; } continue; }
      if (child.kind === 'organisation') { const remote = RemoteOrganisationSchema.parse(payload); const existing = await localDB.canonical_organisations.get(child.id); if (existing && !same(existing, remote)) throw new CanonicalCloudConflictError(`Local organisation ${child.id} differs from remote.`); if (existing) childrenAlreadyLocal++; else { await localDB.canonical_organisations.add(remote); childrenAdded++; } continue; }
      const remote = RemoteRelationshipSchema.parse(payload); const existing = await localDB.canonical_relationships.get(child.id); if (existing && !same(existing, remote)) throw new CanonicalCloudConflictError(`Local relationship ${child.id} differs from remote.`); if (existing) childrenAlreadyLocal++; else { await localDB.canonical_relationships.add(remote); childrenAdded++; }
    }
  });
