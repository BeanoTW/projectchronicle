...
export async function restoreCanonicalOwner(ownerId: string, clock: () => string = () => new Date().toISOString()): Promise<CanonicalRestoreReport> {
  ...
  for (const child of children) {
    ...
    if (child.kind === 'relationship') {
      ...
      if (!remoteEntities.has(`${relationship.entity_type}:${relationship.entity_id}`)) throw new CanonicalCloudConflictError(`Remote relationship ${child.id} points to a missing ${relationship.entity_type}.`);
    }
  }
  ...
  await localDB.transaction('rw', [
    ...
    for (const child of children) {
      ...
      if (child.kind === 'clarification') {
        ...
        if (existing) {
          const { sync: _a, ...existingBody } = existing;
          const { sync: _b, ...remoteBody } = restored;
