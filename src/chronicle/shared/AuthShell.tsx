...
// Record. No V1 hero imagery, gradients or glass styling.
...
interface Props {
  ...
  lede?: ReactNode;
  children: ReactNode;
  /** Optional desktop-only right column (landing uses it). */
  aside?: ReactNode;
  /** Small print under the panel. */
  footer?: ReactNode;
  /** Wider composition for the landing screen. */
  wide?: boolean;
}
  ...
  <div className="proto-root proto-auth" data-auth-shell="true">
    <div className={`proto-auth-grid${wide ? ' proto-auth-grid-wide' : ''}`}>
      <main className="proto-auth-panel">
        <p className="proto-auth-brand proto-serif">Chronicle</p>
      ...
      {aside && <aside className="proto-auth-aside" aria-hidden="false">{aside}</aside>}
    </div>
  </div>
);
