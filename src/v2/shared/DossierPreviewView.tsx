// Phase 6D — source-agnostic dossier preview.
...
const EvidenceMedia = ({ item, useMediaUrl }: { item: DossierEvidenceItem; useMediaUrl: UseMediaUrl }) => {
  const url = useMediaUrl(item);
  if (!url) return null;
  if (item.type === 'image') return <img className="proto-doc-evimg" src={url} alt={item.description ?? item.name} />;
  if (item.type === 'audio') return <audio className="proto-audio proto-noprint" controls src={url} preload="metadata" />;
  return null;
};

const EvidenceBlock = ({ items, useMediaUrl }: { items: DossierEvidenceItem[]; useMediaUrl?: UseMediaUrl }) => (
  <div className="proto-doc-evidence">
    <p className="proto-doc-label">Evidence</p>
    {items.map(e => (
      <div key={e.id} className="proto-doc-evitem">
        {useMediaUrl && <EvidenceMedia item={e} useMediaUrl={useMediaUrl} />}
...
interface Props {
  ...
  /** Hook-shaped resolver for on-screen previews. Must be stable across renders. */
  useMediaUrl?: UseMediaUrl;
}

const DossierPreviewView = ({ doc, cfg, onOpenRecord, useMediaUrl }: Props) => (
      ...
      {doc.records.length === 0 ? (
        ...
        doc.records.map(r => (
          <article key={r.id} className="proto-doc-record">
            ...
            {r.evidence.length > 0 && <EvidenceBlock items={r.evidence} useMediaUrl={useMediaUrl} />}
...
export default DossierPreviewView;
