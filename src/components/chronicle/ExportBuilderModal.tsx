...
const ExportBuilderModal = ({
  ...
  const [config, setConfig] = useState<SequenceConfig>(() => {
    ...
    return revalidateSequences(loaded, incidents.map(i => i.id));
  });
  ...
  useEffect(() => {
    if (open) {
      ...
      const filtered = raw.filter(s =>
        !s.incident_ids.every(id => confirmedIds.has(id))
      );
  ...
  const exportItems = useMemo(
    () => buildExportItems(activeIncidents, config),
    [activeIncidents, config],
  );

  const handleConfirmSuggestion = useCallback((suggestion: SequenceSuggestion) => {
  ...
  const handleRenameSequence = useCallback((seqId: string) => {
    ...
    <div className="fixed inset-0 z-50 bg-background/95 flex flex-col">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-card">
        <div>
          ...
          <p className="text-[12px] text-muted-foreground mt-0.5">
            {activeIncidents.length} incident{activeIncidents.length !== 1 ? 's' : ''} · {config.sequences.length} sequence{config.sequences.length !== 1 ? 's' : ''}
          </p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
        ...
        {suggestions.length > 0 && (
          <div>
            <p className="text-[12px] font-semibold text-muted-foreground mb-2 uppercase tracking-wide">Suggested groupings</p>
            <div className="space-y-2">
              {suggestions.map(s => (
                <div key={s.id} className="bg-card border border-border rounded-xl overflow-hidden">
                  ...
                  >
                    {expandedSuggestion === s.id ? <ChevronDown className="h-4 w-4 text-muted-foreground flex-shrink-0" /> : <ChevronRight className="h-4 w-4 text-muted-foreground flex-shrink-0" />}
                    <div className="flex-1 min-w-0">
                      ...
                      {s.needs_review && (
                        <p className="text-[11px] text-primary mt-0.5 font-medium">Large sequence — review before confirming</p>
                      )}
                    </div>
                  </button>
                  <AnimatePresence>
        ...
        {config.sequences.length > 0 && (
          <div>
            <p className="text-[12px] font-semibold text-muted-foreground mb-2 uppercase tracking-wide">Confirmed sequences</p>
            <div className="space-y-2">
              {config.sequences.map(seq => (
                <div key={seq.id} className="bg-card border border-border rounded-xl overflow-hidden">
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-4 py-3 text-left"
                  >
                    {expandedSequence === seq.id ? <ChevronDown className="h-4 w-4 text-muted-foreground flex-shrink-0" /> : <ChevronRight className="h-4 w-4 text-muted-foreground flex-shrink-0" />}
                    <div className="flex-1 min-w-0">
                      {editingTitle === seq.id ? (
                        <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                          <Input
                            ...
                            onKeyDown={e => { if (e.key === 'Enter') handleRenameSequence(seq.id); }}
                            autoFocus
                          />
                          <Button size="sm" className="h-8 text-[12px]" onClick={() => handleRenameSequence(seq.id)}>Save</Button>
                        </div>
                      ) : (
                        <>
                          <p className="text-[13px] font-semibold text-foreground">{seq.title}</p>
                          <p className="text-[11px] text-muted-foreground">
                            {seq.incident_ids.length} incidents · {seq.source === 'user' ? 'Grouped by user' : 'System suggested'}
                          </p>
                        </>
                      )}
                    </div>
                        ...
                        <div className="px-4 pb-3 space-y-2">
                          ...
                          <div className="flex gap-2 pt-1">
                            <Button
                              ...
                              onClick={() => { setEditingTitle(seq.id); setTitleInput(seq.title); }}
                            >
                              Rename
                            </Button>
                            <Button
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            {!manualSelectMode && standaloneIncidents.length >= 2 && (
              <Button
                ...
                onClick={() => setManualSelectMode(true)}
              >
          ...
          {manualSelectMode && (
            <div className="flex gap-2 mb-3">
              ...
              <Button
                ...
                size="sm"
                className="text-[12px] h-8"
                onClick={() => { setManualSelectMode(false); setSelectedForManual(new Set()); }}
              >
                Cancel
              </Button>
            </div>
