  ...
  return (
    <div className="fixed inset-0 z-50 bg-background/95 backdrop-blur-sm flex flex-col">
      ...
      <div className="px-5 pb-4 flex gap-1.5">
        {[1, 2, 3].map(s => (
          <div key={s} className={`h-1 flex-1 rounded-full transition-colors ${s <= step ? 'bg-primary' : 'bg-muted/40'}`} />
        ))}
      </div>
            ...
            <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.15 }}>
              ...
              <div className="mt-6 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Switch checked={options.includePatterns} onCheckedChange={v => setOptions(o => ({ ...o, includePatterns: v }))} />
                </div>
                <div className="flex items-center justify-between">
                  <label className="text-[13px] text-foreground">Include names</label>
                  <Switch checked={options.includeNames} onCheckedChange={v => setOptions(o => ({ ...o, includeNames: v }))} />
                </div>
              </div>
            </motion.div>
          )}
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.15 }}>
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <button onClick={regenerate} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted/40 text-muted-foreground text-[12px] font-medium hover:bg-muted/60">
