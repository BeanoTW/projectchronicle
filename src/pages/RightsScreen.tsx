...
import { mockRightsGuidance, mockIncidents } from '@/data/mockData';
import EmptyState from '@/components/chronicle/EmptyState';

const RightsScreen = () => {
  const userCategories = useMemo(() => {
    const cats = new Set<string>();
    mockIncidents.forEach(i => { if (i.category) cats.add(i.category); });
    return cats;
  }, []);

  const relevantGuidance = mockRightsGuidance.filter(r => r.active && userCategories.has(r.incident_category));
  const allGuidance = mockRightsGuidance.filter(r => r.active).sort((a, b) => a.display_order - b.display_order);
