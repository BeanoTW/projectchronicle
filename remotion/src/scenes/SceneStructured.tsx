...
const Field: React.FC<{ label: string; value: React.ReactNode; delay: number }> = ({ label, value, delay }) => {
  ...
  return (
    <div style={{ opacity, transform: `translateY(${ty}px)` }}>
      <div style={{ fontSize: 12, color: COLOR.textMuted, textTransform: 'uppercase', letterSpacing: '0.10em', fontWeight: 600, marginBottom: 8 }}>{label}</div>
      <div style={{ fontSize: 17, color: COLOR.text, fontWeight: 500, lineHeight: 1.35 }}>{value}</div>
    </div>
  );
};
...
export const SceneStructured: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: COLOR.bg, fontFamily: FONT.ui }}>
      ...
      <div style={{ margin: '0 28px', padding: '26px 26px 28px', background: COLOR.surface, border: `1px solid ${COLOR.border}`, borderRadius: 18, display: 'flex', flexDirection: 'column', gap: 22 }}>
        ...
        <Field
          ...
          delay={20}
          value={
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 999, background: COLOR.primarySoft, border: `1px solid ${COLOR.primary}55`, color: COLOR.primaryDark, fontSize: 13, fontWeight: 600 }}>
              Workplace
            </div>
          }
        />
        ...
        <Field
          ...
          delay={38}
          value={
            <div style={{ fontSize: 16, color: COLOR.text, fontWeight: 400, lineHeight: 1.55 }}>
